...
const getCache = (key) => 
  { const cached = localStorage.getItem(key); 
    if (!cached) return null; const parsed = JSON.parse(cached); 
    if (Date.now() > parsed.expiry) { localStorage.removeItem(key); 
      return null; 
    } return parsed.data; 
  };
...
function useUnverifiedStudents(collegeId) {
  ...
  const fetchRows = useCallback(async () => {
    if (!collegeId) { setRows([]); setLoading(false); return; }
    setLoading(true); setErr("");
    try {
      const q = query(collection(db, "Students"), where("collegeId", "==", collegeId), where("role", "==", "unverified"));
      const querySnapshot = await getDocs(q);
      if (querySnapshot.empty) { 
        console.log(`Firestore query for collegeId "${collegeId}" returned no documents.`); 
        setRows([]); 
      } else {
      ...
      setErr("Failed to load unverified students from Firestore: " + e.message); 
...
function CustomNoRowsOverlay() { return ( <GridOverlay><Box sx={{ mt: 1 }}>No unverified students found.</Box></GridOverlay> ); }

export default function StudentsVerify() {
  const theme = useTheme();
  const { mode } = useContext(ThemeContext);
  const { role, userDetails, loading: authLoading } = useContext(AuthContext);
  const isCollegeAssociate = userDetails?.isCollegeAssociate || false;
  const associateCollegeId = userDetails?.college || null;

  if (authLoading) { return ( <Box sx={{ p: 3, display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh" }}><CircularProgress color="primary" /></Box> ); }
  ...
  const [addData, setAddData] = useState({ semester: "", yearOfAdmission: new Date().getFullYear().toString(), program: "", department: "", enrollmentNo: "", firebaseId: "" });
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      ...
      if (!data) {
        ...
        } catch (error) { console.error("Failed to fetch departments:", error); data = []; }
      }
      setDepartments(data || []);
    };
    fetchDepartments();
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      if (!addData.department || !associateCollegeId) { setPrograms([]); return; }
      ...
      if (!data) {
        try {
          ...
          data = res.data;
          setCache(cacheKey, data);
        } catch (error) { console.error("Failed to fetch programs:", error); data = []; }
      }
      setPrograms(data || []);
    };
    fetchPrograms();
  ...
  useEffect(() => {
    const fetchSemesters = async () => {
      if (!addData.program) { setSemesters([]); return; }
      ...
      if (!data) {
        try {
          ...
          data = res.data;
          setCache(cacheKey, data);
        } catch (error) { console.error("Failed to fetch semesters:", error); data = []; }
      }
      setSemesters(data || []);
    };
    fetchSemesters();
  ...
  const accessDenied = !(role === "CollegeAssociate" || isCollegeAssociate);
  const filtered = useMemo(() => {
    let data = rows;
    const q = search.trim().toLowerCase();
    if (q) {
      data = data.filter((r) => {
        const fields = [r.firstName, r.lastName, r.email, r.phone, r.abcId, r.collegeName, r.gender].filter(Boolean);
        return fields.some((f) => String(f).toLowerCase().includes(q));
      });
    }
    if (genderFilter) { data = data.filter((r) => (r.gender || "").toLowerCase() === genderFilter.toLowerCase()); }
    return data;
  ...
  const columns = useMemo(() => [
    ...
    { field: "collegeName", headerName: "College", flex: 1.5, minWidth: 150, renderCell: (params) => ( <Tooltip title={params.value || ""}>
      ...
      renderCell: (params) => ( 
        <Button variant="contained" size="small" startIcon={<AddIcon />} 
                onClick={() => { 
                  ...
                  setAddDataOpen(true); }}>Add Data</Button> 
                ) }
  ], [associateCollegeId]);
  ...
  const handleAddAndVerify = async () => {
    ...
    if (errorMsg) { setToast({ open: true, type: "error", msg: errorMsg }); return; }
    setBusy(true);
    try {
      let exists = false;
      try {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "enrollmentNo") 
      { if (!enrollmentPrefix) return; 
        ...
        next = enrollmentPrefix + suffix; setAddData((prev) => ({ ...prev, enrollmentNo: next })); 
        return; }
    if (name === "department") { 
    ...
    {{ background: mode === "default" ? `linear-gradient(135deg, ${theme.palette.green.main} -50%, ${theme.palette.green.focus} 90%)` 
      ...
      <Card sx={
        ...
        <Stack direction={{ xs: "column", md: "row" }} 
          ...
          <TextField variant="outlined" size="small" placeholder="Search name, email fryer, email, phone, ABC ID..." 
                      value={search} 
        ...
        {err && ( <Alert sx={{ mb: 3, borderRadius: 2 }} severity="error">{err}</Alert> )}
        <Paper elevation={1} 
        ...
        >
          <div style={{ width: "100%" }}>
            <DataGrid 
                     ...
                     sx={{ 
                          border: "none", "& .MuiDataGrid-columnHeaders": 
      ...
      <Dialog open={addDataOpen} 
              ...
              maxWidth="sm" 
              fullWidth PaperProps={{ sx: { borderRadius: 3, p: 1 } 
              }}>
        <DialogTitle 
          sx={{ fontWeight: 900 }}>
            Add Data<IconButton 
              ...
              <Stack spacing={2} sx={{ mt: 2 }}>
                    ...
                    <InputLabel>Program</InputLabel>
                      <Select name="program" 
                              ...
                              onKeyDown={(e) => handleKeyDown(e, semesterRef)}>
                                <MenuItem value="" 
                                          disabled>Select Program
                                </MenuItem>{programs.map((prog) => ( 
                                            <MenuItem key={prog._id} 
                                                      value={prog._id}>{prog.programName}
                                            </MenuItem> 
                                          ))}
                      </Select>
                  </FormControl>
                ...
                <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
                  <TextField label="Year of Admission" 
                             ...
                             fullWidth inputProps={{ inputMode: "numeric", pattern: "[0-9]*", maxLength: 4 }} 
                             helperText="Format: YYYY" 
                  />
                  <TextField label="Enrollment No Suffix" 
                             name="enrollmentNo" 
                             value={addData.enrollmentNo.substring(enrollmentPrefix.length)} 
                             onChange={(e) => { const suffix = e.target.value.replace(/\D/g, "").slice(0, 3); 
                              setAddData((prev) => ({ 
                                ...prev, 
                              ...
                              fullWidth helperText="Enter last 3 digits" 
                              InputProps={{ startAdornment: enrollmentPrefix ? ( 
        ...
        <DialogActions sx={{ pt: 2 }}>
          ...
          <Button color="success" 
                  ...
                  disabled={busy || !addData.semester || !addData.yearOfAdmission || !addData.program || !addData.department || !addData.enrollmentNo || (enrollmentPrefix && addData.enrollmentNo.length !== enrollmentPrefix.length + 3)} 
